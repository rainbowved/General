...
17.0.1 MINIMUM_CONTENT_SCHEMA (DATA, DB_SPEC) — MVP gate
[DATA][DB_SPEC] Минимальная схема контент-пака (authoring → runtime)
• tag_registry: реестр tag_id (см. 17.0.2; определение полей — в B18 TAG_REGISTRY addendum).
• class_registry: реестр class_id (см. 17.0.2; определение полей — в B18 TAG_REGISTRY addendum).
• tier_axis: единая ось tier_id="T1".."T5" + маппинги (см. 17.0.3 и B11 ACCESS_TIER).
• region_def: регион мира (region_id, name_key, tags[]).
• route_node_def: узел маршрута/графа экспедиции (route_node_id, region_id, kind, tags[], tier_id, adjacency_refs?).
• item_def: предметы (см. B14 DB_SPEC::items).
• monster_def: монстры (см. B16/B19; ELITE использует derived MON_*_ELITE, без рантайм-генерации).
• encounter_def: состав энкаунтера (encounter_id, members[], tags[], tier_id, reward_coffer_id?).
• spawn_table_def: таблица кандидатов энкаунтера (spawn_table_id, candidates[], weight_int, tag-filters, tier_id).
• poi_def: POI-паттерны/инстансы (см. 17.2+).
• loot_pool_def / loot_row_def / coffer_def: коферы/пулы/строки лута (см. B24).

[CANON] Ссылочная целостность и fail-fast (MVP)
• Любой *_id ref обязан указывать на существующую запись (registry/def). Unknown → build-step FAIL и load-time FAIL-FAST.
• Canonicalization для списков: tags_all/tags_any/tags_none и любые "unordered" массивы сортируются asc при сериализации/хешах; семантические массивы (например slot_pool_ids) НЕ сортируются.
• Build-step обязан фиксировать/публиковать alias→canonical_id карты, не меняя immutable *_id (см. DB_FILL_PATCH::CONTENT_ID_IMMUTABILITY).

17.0.2 WORLD_IDENTITY (DATA, DB_SPEC) — region_id / route_node_id
[DATA][DB_SPEC] region_def
• region_id:string (immutable).
• name_key:string (i18n).
• tags[]: tag_id[] (опц.; сортируются asc).
[DATA][DB_SPEC] route_node_def
• route_node_id:string (immutable; уникален глобально или в пределах region_id — выбрать одно и закрепить в DB_SPEC).
• region_id:string ref.
• kind:string (например "road" | "poi_node" | "choke" | "shelter" | "cache" | ...; data-only enum).
• tier_id:string ref (T1..T5; см. 17.0.3).
• tags[]: tag_id[] (опц.; сортируются asc).
• adjacency_refs[]: route_node_id[] (опц.; если используется граф; порядок семантический, НЕ сортировать).
[CANON] Требование для детерминизма/anti-reroll
• scope_id для pick в spawn/loot обязан включать route_node_id (см. 17.1 POI_INSTANCE_ID; аналогично для encounter_instance_id).

17.0.3 TIER_AXIS (CANON + DATA) — единая ось тиров
[CANON] Canonical tier_id
• Единственный канонический идентификатор тира контента: tier_id ∈ {"T1","T2","T3","T4","T5"}.
• access_tier (save.progress) использует тот же набор значений (см. B11 ACCESS_TIER).
• Любые альтернативные шкалы/лейблы (tier_band, Grade G1..G5/A1..A5 и т.п.) — строго data-only и обязаны маппиться на tier_id через tier_axis mapping (без параллельных осей в правилах).
[DATA][DB_SPEC] tier_axis
• tier_id:string (T1..T5) (immutable).
• label_key:string (i18n).
• (опц.) difficulty_band:string / loot_grade_band:string / access_requirements_ref:string — только если реально используется в клиентах/валидаторах.
• (опц.) aliases[]: string[] — разрешённые alias-значения (например "1","2","3","4","5" или "tier_1") для authoring; build-step нормализует в tier_id.
...
• tier_id: string ("T1".."T5") — canonical tier reference (см. B11 ACCESS_TIER).
• tier: int (1..5) — authoring shorthand; build-step обязан нормализовать в tier_id (1→T1..5→T5).
...
[DATA][DB_SPEC] tag_registry (MVP)
• tag_id:string (immutable).
• domain:string (например "item" | "monster" | "poi" | "loot" | "quest" | "service" | "world" | ...).
• desc_key:string? (i18n; подсказка для инструментов/валидаторов; не обязателен для runtime).
• flags_int:int? (опц.; битмаска/атрибуты).
[DATA][DB_SPEC] class_registry (MVP)
• class_id:string (immutable).
• domain:string (как выше).
• desc_key:string? (i18n).
• flags_int:int? (опц.).
[DATA] allowlist_tag_namespaces_by_subsystem (MVP)
• subsystem_id: "QUESTS" | "SHOPS" | "EXCHANGE" | "CRAFT" | "LOOT" | "IDENTIFY" | "REPAIR" | "SALVAGE" | "POI" | "SPAWN".
• allow_namespaces[]: string[] (например ["quest","archive","item","status","service"]).
• Правило: при разборе/применении tags_* фильтров подсистема учитывает только tag_id с namespace из allowlist; иначе reject на build-step (не UI-игнор).
...
• LOOT_POOL_DEF: row_id уникален в пределах pool_id; qty>=1; weight_int
• LOOT_ROW_DEF.spawn_tags_add tag_id должны существовать в tag_registry; unknown → fail-fast.
• LOOT_ROW_DEF.force_unidentified=true допустим только если создаваемая сущность поддерживает UNIDENTIFIED (валидация на item_def/flags).
...
[DATA NOTICE] Grade (G1..G5/A1..A5) — data-only шкала; любые привязки к tier_id обязаны проходить через tier_axis mapping (см. 17.0.3), не держать параллельную ось в правилах.
